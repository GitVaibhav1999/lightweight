...
await sleep(4000);
console.log(`launch+calendar: ${Date.now()-t0}ms`);
for (let i=0;i<6;i++){
  const t=Date.now();
  await d.execute('mobile: swipe',{direction:'up'});
  await sleep(350);
  console.log(`  swipe ${i+1}: ${Date.now()-t}ms`);
}
const src = await d.getPageSource();
const m = src.match(/(\d+) older/);
console.log('sentinel:', m ? `"${m[0]}" present` : 'not present (all loaded)');
console.log('history header:', (src.match(/All history · (\d+) sessions/)||['?'])[0]);
await d.saveScreenshot('./verify/history.png');
